...
from climlab_stardust_extension.dynamics import two_d_adv_diff_numerics as numerics
...
class TwoDimensionalAdvectionDiffusion(TimeDependentProcess):
    ...
    def _mixed_diffusion_to_advection(self):
        ...
        if np.max(np.abs(self.Kyz)) > 0.0:
            ...
            self._Ud = np.divide(k_y_zmid * dqdz, tracer_m, out=np.zeros_like(k_y_zmid, dtype=float), where=(tracer_m != 0))
        ...
        if np.max(np.abs(self.Kyz)) > 0.0:
            ...
            # avoid division by zero:
            self._Wd = np.divide(k_ymid_z * dqdy, tracer_m, out=np.zeros_like(k_ymid_z, dtype=float), where=(tracer_m != 0))
